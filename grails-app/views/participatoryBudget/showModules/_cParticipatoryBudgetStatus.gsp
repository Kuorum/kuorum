        ...
        <ul class="participatory-budget-steps-info">
            <g:each in="${org.kuorum.rest.model.communication.participatoryBudget.ParticipatoryBudgetStatusDTO.values()}" var="status" status="i">
                <li class=" kuorum-tooltip ${status == participatoryBudget.status?'active':''}"
                    ...
                    <g:if test="${status == org.kuorum.rest.model.communication.participatoryBudget.ParticipatoryBudgetStatusDTO.CLOSED}"><g:set var="statusDate"><g:formatDate type="date" style="small" date="${participatoryBudget.deadLineResults}"/></g:set><g:set var="statusDateMsgCode" value="participatoryBudget.status.from"/></g:if>
