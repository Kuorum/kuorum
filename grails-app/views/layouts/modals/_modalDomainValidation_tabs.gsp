...
<g:set var="numberValidationsActive"
       value="${(_domainValidations?.phone ? 2 : 0) + (_domainValidations?.census ? 1 : 0) + (_domainValidations?.customCode ? 1 : 0) + (_domainValidations?.tokenMail ? 1 : 0)}"/>
...
%{-- Only shows steps if the phone is active because if phone is not active the census doesn't need the step bullets--}%
    <ol class="modal-domain-validation-step-tabs stepsSign">
        <g:if test="${_domainValidations?.tokenMail}"><li class="modal-domain-validation-step-tabs-tokenMail"><div
                class="step-label"><g:message code="kuorum.web.commands.profile.tokenMail.title"/></div></li></g:if>
