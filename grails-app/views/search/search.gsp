...
<head>
    <title><g:message code="page.title.search"
                      args="[searchParams.word ?: '', kuorum.core.customDomain.CustomDomainResolver.domainRSDTO.name]"/></title>
...
<content tag="leftMenu">
    ...
    <ul class="search-options">
        <li>
            <label>
                <g:link mapping="searcherSearch"
                        params="${params.findAll { k, v -> k != 'solrType' && k != 'offset' && v }}"
                        class="${!searchParams.solrType ? 'search-option-active' : ''}">
        ...
        <g:if test="${_isSocialNetwork}">
            <li>
                <label>
                    <g:link mapping="searcherSearchKUORUM_USER"
                            params="${params.findAll { k, v -> k != 'solrType' && k != 'offset' && v }}"
                            class="${searchParams.solrType == kuorum.core.model.solr.SolrType.KUORUM_USER ? 'search-option-active' : ''}">
        ...
        <g:each in="${activeSearchableCampaigns}" var="activeSolrType">
            <li>
                <label>
                    <g:link mapping="${'searcherSearch' + activeSolrType.toString()}"
                            params="${params.findAll { k, v -> k != 'solrType' && k != 'offset' && v }}"
                            class="${searchParams.solrType == activeSolrType ? 'search-option-active' : ''}">
        ...
        <ul class="causes-tags">
            <g:each in="${relevantCauses}" var="tag">
                <li class="cause link-wrapper active" id="cause-participacion">
                    <g:link mapping="searcherSearchByCAUSE"
                            params="${params.findAll { k, v -> k != 'searchType' && k != 'offset' && v } + [word: tag.name]}"
                            class="sr-only hidden">Search cause ${tag.name}</g:link>
