...
<li class="${newsletter.status}" id="campaignPos_${idx}">
    ...
    <p class="name">
        <g:message code="${org.kuorum.rest.model.notification.campaign.CampaignStatusRSDTO.class.name}.${newsletter.status}"/>
        <input class="timestamp" type="hidden" val="${newsletter?.sentOn?.time?:'LAST'}" />  %{-- Letters are after than numbers--}%
        <g:if test="${newsletter.sentOn}">
            <span class="date">
                (<g:formatDate date="${newsletter.sentOn}" type="datetime" style="LONG" timeStyle="SHORT" timeZone="${user.timeZone}"/>)
            </span>
    ...
    <ul class="list-actions">
        ...
        <li>
            <g:link mapping="politicianMassMailingCopy" params="[campaignId:bulletin.id]" class="campaignStats"><span class="fal fa-copy"></span> <span class="sr-only">Copy</span></g:link>
        </li>
